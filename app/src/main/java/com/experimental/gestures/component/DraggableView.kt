...
/*
class DraggableCustomView(context: Context, attrs: AttributeSet) : View(context, attrs) ,
    View.OnTouchListener {
    ...
    private fun transition(event: DraggableViewEvent) {
        when (state) {
            DraggableViewState.IDLE -> {
                when (event) {
                    DraggableViewEvent.TOUCH_DOWN -> {
                        state = DraggableViewState.DRAGGING
    ...
    override fun onTouch(view: View, event: MotionEvent): Boolean {
        // Handle touch events using the same code as in the onTouchEvent method
        // ...
    }
}
